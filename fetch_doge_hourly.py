"""Fetch DOGE hourly OHLC from CoinMarketCap data-api in 400-hour chunks.

Range: 2020-12-26 → 2021-06-03 UTC (Elon Musk SNL rally penceresi).
CMC returns at most 400 hourly rows per request, anchored to timeEnd. We walk
forward in time, advancing timeEnd by ~16 days each step, dedup by hour
timestamp, and write to data/doge_hourly.json.
"""
import urllib.request, json, time, datetime as dt, os, sys

CMC_ID = 74             # DOGE
CONVERT_ID = 2781       # USD
OUT = os.path.join(os.path.dirname(__file__), 'data', 'doge_hourly.json')

START = dt.datetime(2020, 12, 26, 0, 0, tzinfo=dt.timezone.utc)
END   = dt.datetime(2021, 6, 3, 23, 0, tzinfo=dt.timezone.utc)
...
def main():
    ...
    while cursor < END + dt.timedelta(hours=CHUNK_HOURS):
        ...
        for q in quotes:
            ...
            # Range filter: keep only the requested window
            if t < START.strftime('%Y-%m-%dT%H:00') or t > END.strftime('%Y-%m-%dT%H:00'):
                continue
            qq = q.get('quote', {})
            p = qq.get('close')
            v = qq.get('volume')
